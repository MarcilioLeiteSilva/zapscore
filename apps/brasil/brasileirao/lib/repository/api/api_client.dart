...
class ApiClient {
  ...
  final Map<String, String> _resolvedLeagueUuids = {};
  ...
  Future<String?> _resolveLeagueIdToUuid(String? rawLeagueId) async {
    ...
    if (_resolvedLeagueUuids.containsKey(rawLeagueId)) {
      return _resolvedLeagueUuids[rawLeagueId];
    }
    ...
    try {
      ...
      for (final l in leagues) {
        if (l.id == rawLeagueId || (targetExtId != null && l.externalId == targetExtId)) {
          _resolvedLeagueUuids[rawLeagueId] = l.id;
          return l.id;
        }
      }
    } catch (_) {}
    ...
    if (response.statusCode == 200) {
      ...
      if (data is List) {
        ...
        return list.where((f) {
          ...
          return extId == targetLeagueId || id == '$targetLeagueId' || AppConfig.supportedLeagueIds.contains(extId) || name.contains('brasileir');
        }).toList();
      }
      return [];
    } else {
