  ...
  Future<void> _loadLeagues() async {
    try {
      ...
      final filtered = list.where((league) {
        ...
        return AppConfig.supportedLeagueIds.contains(extId) || name.contains('brasileir');
      }).toList();

      bool hasBrasileirao = filtered.any((l) => AppConfig.supportedLeagueIds.contains(l.externalId) || l.name.toLowerCase().contains('brasileir'));

      if (!hasBrasileirao) {
        filtered.insert(0, League(id: '71', externalId: 71, name: 'Brasileirão Série A', country: 'Brazil'));
        filtered.add(League(id: '72', externalId: 72, name: 'Brasileirão Série B', country: 'Brazil'));
      }
      ...
      setState(() {
        _leagues = [
          League(id: '71', externalId: 71, name: 'Brasileirão Série A', country: 'Brazil'),
          League(id: '72', externalId: 72, name: 'Brasileirão Série B', country: 'Brazil'),
        ];
      });
    }
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      AiPerformanceStats data = await _apiClient.getAiPerformanceStats(
        leagueId: _selectedLeague?.externalId ?? AppConfig.externalLeagueId,
        days: _selectedDays,
      );
      ...
      if (data.recentAudits.isNotEmpty) {
        final filteredAudits = data.recentAudits.where((audit) {
          ...
          return lName.contains('brasileir') || lName.contains('série') || lName.contains('serie');
        }).toList();
