    ...
    return BlocBuilder<LeagueCubit, LeagueState>(
      builder: (context, state) {
        ...
        if (state is LeagueLoaded) {
          ...
          // 2. Semifinais: exibe jogos agrupados por confronto, sem placar agregado
    ...
    return BlocBuilder<LeagueCubit, LeagueState>(
      builder: (context, state) {
        ...
        if (state is LeagueLoaded) {
          ...
          // 4. Pontos Corridos / Liga Única (Brasileirão Série A / Série B)
          if (activePhase != null && activePhase.isLeague) {
            final isSerieB = state.selectedLeagueId == AppConfig.brasileiraoSerieBId;
            return ListView(
              padding: const EdgeInsets.only(left: 10, right: 10, top: 15, bottom: 120),
              children: [
                StandingsTableWidget(
                  ...
                  qualificationCount: 4,
                  relegationCount: 4,
                  qualificationDescription: isSerieB
                      ? '1º a 4º colocados garantem o acesso à Série A.'
                      : '1º a 4º colocados garantem vaga na fase de grupos da Libertadores.',
                  relegationDescription: isSerieB
                      ? '17º a 20º colocados são rebaixados para a Série C.'
                      : '17º a 20º colocados são rebaixados para a Série B.',
                ),
              ],
            );
          }
