...
class SearchCubit extends Cubit<SearchState> {
  ...
  final Map<int, Team> _brasileiraoTeamsMap = {};
  final Set<String> _brasileiraoTeamNames = {};
  static const List<int> supportedLeagueIds = AppConfig.supportedLeagueIds; // [71, 72]

  SearchCubit(this.apiClient) : super(SearchInitial());

  Future<void> _loadBrasileiraoTeams() async {
    if (_brasileiraoTeamsMap.isNotEmpty) return;
    for (var lId in supportedLeagueIds) {
      try {
        final st = await apiClient.getStandings(lId);
        for (var s in st) {
          if (s.teamId > 0) {
            _brasileiraoTeamsMap[s.teamId] = Team(
            ...
            if (s.teamName.isNotEmpty) {
              _brasileiraoTeamNames.add(s.teamName.toLowerCase());
            }
          }
        }
      } catch (_) {}
      ...
      try {
        ...
        for (var f in fix) {
          if (f.homeTeam != null) {
            ...
            if (hId > 0) {
              _brasileiraoTeamsMap[hId] = f.homeTeam!;
              if (f.homeTeam!.name.isNotEmpty) {
                _brasileiraoTeamNames.add(f.homeTeam!.name.toLowerCase());
              }
            }
          }
          if (f.awayTeam != null) {
            ...
            if (aId > 0) {
              _brasileiraoTeamsMap[aId] = f.awayTeam!;
              if (f.awayTeam!.name.isNotEmpty) {
                _brasileiraoTeamNames.add(f.awayTeam!.name.toLowerCase());
              }
            }
          }
        }
  ...
  Future<void> search(String query) async {
    ...
    if (cleanQuery.isEmpty) {
      emit(SearchLoading());
      try {
        await _loadBrasileiraoTeams();
        final leagues = await apiClient.getStoredLeagues();
        final brasileiraoLeagues = leagues
            .where((l) =>
                supportedLeagueIds.contains(l.externalId) ||
                l.name.toLowerCase().contains('brasileir'))
            .toList();
        emit(SearchLoaded(
          teams: _brasileiraoTeamsMap.values.toList(),
          leagues: brasileiraoLeagues.isNotEmpty ? brasileiraoLeagues : leagues,
          fixtures: [],
        ));
      } catch (e) {
        emit(SearchInitial());
    ...
    try {
      ...
      try {
        ...
        // 1. Filtrar nos times conhecidos do Brasileirão Séries A e B
        final Map<int, Team> filteredTeams = {};
        for (var team in _brasileiraoTeamsMap.values) {
        ...
        // 2. Buscar times na API e filtrar estritamente pelos times do Brasileirão
        try {
          ...
          for (var t in rawTeams) {
            ...
            final bool isBrasileirao = (_brasileiraoTeamsMap.containsKey(extId)) ||
                _brasileiraoTeamNames.any((cn) =>
                    cn == tName ||
      ...
      try {
        ...
        fixtures = rawFixtures.where((f) {
          ...
          return extId != null && supportedLeagueIds.contains(extId);
        }).toList();
      } catch (e) {
