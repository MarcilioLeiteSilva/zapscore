...
class LiveCubit extends Cubit<LiveState> {
  ...
  Future<void> fetchLiveFixtures({bool isAutoRefresh = false}) async {
    ...
    try {
      ...
      const supportedIds = AppConfig.supportedLeagueIds;
      final liveFixtures = fixtures.where((f) {
        final extId = f.league?.externalId;
        final id = f.league?.id;
        final name = f.league?.name.toLowerCase() ?? '';
        return (extId != null && supportedIds.contains(extId)) ||
            supportedIds.any((cid) => id == '$cid') ||
            name.contains('brasileir');
      }).toList();
      emit(LiveLoaded(liveFixtures));
    } catch (e) {
      if (!isAutoRefresh) {
