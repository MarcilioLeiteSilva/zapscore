...
export default function BrasilModulePage() {
  ...
  return (
    <div className="space-y-[30px]" style={{ fontFamily: 'var(--font-outfit)' }}>
        ...
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
              Módulo <span style={{ color: 'var(--primary)' }}>Brasil</span>
            </h1>
            <span className="badge badge-ft">• ZAPSCORE API ACTIVE</span>
          </div>
          <p className="text-[var(--text-muted)] text-sm mt-2 max-w-2xl">
            Gerenciador Central de Notícias, Vídeos e Artilharia do Campeonato Brasileiro (Brasileirão Série A e Série B).
          </p>
        </div>
      ...
      {/* Grid de Competições Cobertas */}
      <div className="space-y-[30px]">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-white flex items-center gap-3">
            <Trophy size={20} className="text-amber-400" />
            <span>Competições Nacionais Cobertas</span>
            <span className="text-xs font-bold px-3 py-1 rounded-full bg-[var(--surface-hover)] text-[var(--text-muted)] border border-[var(--border)]">
              {brasilModule?.leagues.length || 2} Competições
            </span>
          </h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[30px]">
          {brasilModule?.leagues.map((league) => (
            <Link
              key={league.id}
              href={`/adminpanel/brasil/${league.id}`}
              onClick={() => router.push(`/adminpanel/brasil/${league.id}`)}
