      ...
      {fixtures.length === 0 ? (
        ...
        <div className="grid-fixtures">
          {fixtures.map((fixture) => (
            <div key={fixture.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: 'var(--muted)' }}>
                <span>{fixture.league.name} — Rodada {fixture.league.round || '?'}</span>
                <span className={`badge badge-${fixture.statusShort?.toLowerCase()}`}>{fixture.statusLong}</span>
              </div>
