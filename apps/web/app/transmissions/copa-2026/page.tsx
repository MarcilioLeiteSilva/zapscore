...
const overlays = [
  { slug: 'scoreboard', name: 'Placar ao Vivo', icon: '⚽', desc: 'Placar, seleções e status da partida em tempo real' },
  { slug: 'events', name: 'Eventos', icon: '📋', desc: 'Gols, cartões e substituições ao vivo' },
  { slug: 'lineups', name: 'Escalações', icon: '👥', desc: '11 titulares de cada seleção' },
  { slug: 'stats', name: 'Estatísticas', icon: '📊', desc: 'Posse, finalizações, escanteios e mais (scroll loop)' },
  { slug: 'standings', name: 'Classificação', icon: '🏆', desc: 'Tabela de posições / grupos (scroll loop)' },
];

export default function Copa2026Hub() {
  return (
    <div className="tx-hub">
      ...
      <h1>
        🌍 Copa do Mundo <span style={{ color: 'var(--tx-primary)' }}>FIFA 2026</span>
      </h1>
      <p style={{ color: 'var(--tx-text-muted)', fontSize: '1rem', marginBottom: '1rem' }}>
        League ID: 1 • Selecione um overlay para usar no OBS
      </p>
      ...
      <div className="tx-hub-grid">
        {overlays.map((overlay) => (
          <Link
            key={overlay.slug}
            href={`/transmissions/copa-2026/${overlay.slug}`}
