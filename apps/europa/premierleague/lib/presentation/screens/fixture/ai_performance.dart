  ...
  Future<void> _loadLeagues() async {
    try {
      ...
      final filtered = list.where((league) {
        ...
        return extId == 39 || name.contains('premier league') || name.contains('premierleague');
      }).toList();

      bool hasPremierLeague = filtered.any((l) => l.externalId == 39 || l.name.toLowerCase().contains('premier league') || l.name.toLowerCase().contains('premierleague'));

      if (!hasPremierLeague) {
        filtered.insert(0, League(id: '39', externalId: 39, name: 'Premier League', country: 'England'));
      }
      ...
      setState(() {
        _leagues = [
          League(id: '39', externalId: 39, name: 'Premier League', country: 'England'),
        ];
      });
    }
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      AiPerformanceStats data = await _apiClient.getAiPerformanceStats(
        leagueId: _selectedLeague?.externalId ?? 39,
        days: _selectedDays,
      );
      ...
      if (data.recentAudits.isNotEmpty) {
        final filteredAudits = data.recentAudits.where((audit) {
          ...
          return lName.contains('premier league') || lName.contains('premierleague');
        }).toList();
