...
class _WatchPageState extends State<WatchPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<VideoCubit>().fetchVideos(leagueId: AppConfig.leagueId, limit: 100);
    });
  }

  bool _matchesFilter(Video video, String? filter) {
    if (filter == null || filter.isEmpty || filter == 'Premier League') return true;
  ...
  List<Map<String, String?>> _generateDynamicCategories(List<Video> allVideos, HomeState homeState) {
    final List<Map<String, String?>> categories = [
      {'name': 'Premier League', 'filter': null},
    ];
    ...
    final Set<String> addedNames = {'Premier League'};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () => context.read<VideoCubit>().fetchVideos(leagueId: AppConfig.leagueId, limit: 100),
