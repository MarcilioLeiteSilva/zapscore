...
class LiveCubit extends Cubit<LiveState> {
  ...
  Future<void> fetchLiveFixtures({bool isAutoRefresh = false}) async {
    ...
    try {
      final fixtures = await apiClient.getLiveFixtures(leagueId: 39);
      final premierLeagueFixtures = fixtures.where((f) {
        final extId = f.league?.externalId;
        final id = f.league?.id;
        final name = f.league?.name.toLowerCase() ?? '';
        return extId == 39 || id == '39' || name.contains('premier league') || name.contains('premierleague');
      }).toList();
      emit(LiveLoaded(premierLeagueFixtures));
    } catch (e) {
      if (!isAutoRefresh) {
