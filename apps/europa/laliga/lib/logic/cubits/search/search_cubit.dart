  ...
  Future<void> search(String query) async {
    if (query.isEmpty) {
      ...
      try {
        ...
        final laligaLeagues = leagues.where((l) => l.externalId == laligaId).toList();
        emit(SearchLoaded(
          teams: [],
          leagues: laligaLeagues.isNotEmpty ? laligaLeagues : leagues,
          fixtures: [],
        ));
      } catch (e) {
        emit(SearchInitial());
    ...
    try {
      ...
      try {
        ...
        teams = rawTeams.where((t) {
          ...
          return country == 'spain' || country == 'espanha';
        }).toList();
      } catch (e) {
      ...
      try {
        ...
        leagues = rawLeagues.where((l) => l.externalId == laligaId || l.name.toLowerCase().contains('la liga') || l.name.toLowerCase().contains('laliga')).toList();
      } catch (e) {
