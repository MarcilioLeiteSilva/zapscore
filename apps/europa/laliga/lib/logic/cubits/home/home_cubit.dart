...
class HomeCubit extends Cubit<HomeState> {
  ...
  Future<void> fetchHomeData({DateTime? date}) async {
    ...
    try {
      ...
      const laligaId = 140;
      final targetLeagues = leagues.where((l) => l.externalId == laligaId).toList();
      ...
      if (targetLeagues.isNotEmpty) {
        ...
        // Fallback se getStoredLeagues retornar lista ainda sem ID 140
        List<Fixture> matches = await apiClient.getFixturesByDate(laligaId, formattedDate);
        final dummyLeague = leagues.firstWhere((l) => l.externalId == laligaId, orElse: () => leagues.first);
