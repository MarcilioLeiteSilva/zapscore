...
class _WatchPageState extends State<WatchPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<VideoCubit>().fetchVideos(leagueId: '140', limit: 100);
    });
  }

  bool _matchesFilter(Video video, String? filter) {
    if (filter == null || filter.isEmpty || filter == 'La Liga') return true;
  ...
  List<Map<String, String?>> _generateDynamicCategories(List<Video> allVideos, HomeState homeState) {
    ...
    final Set<String> addedNames = {'La Liga'};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () => context.read<VideoCubit>().fetchVideos(leagueId: '140', limit: 100),
