  ...
  Future<void> _loadLeagues() async {
    try {
      ...
      final filtered = list.where((league) {
        ...
        return extId == targetExtId || name.contains('serie a') || name.contains('seriea');
      }).toList();

      bool hasSerieA = filtered.any((l) => l.externalId == targetExtId || l.name.toLowerCase().contains('serie a') || l.name.toLowerCase().contains('seriea'));

      if (!hasSerieA) {
        filtered.insert(0, League(id: AppConfig.leagueId, externalId: targetExtId, name: 'Serie A', country: 'Italy'));
      }
      ...
      setState(() {
        _leagues = [
          League(id: AppConfig.leagueId, externalId: AppConfig.externalLeagueId, name: 'Serie A', country: 'Italy'),
        ];
      });
    }
  }
    ...
    try {
      ...
      if (data.recentAudits.isNotEmpty) {
        final filteredAudits = data.recentAudits.where((audit) {
          ...
          return lName.contains('serie a') || lName.contains('seriea');
        }).toList();
