...
class HomeCubit extends Cubit<HomeState> {
  ...
  Future<void> fetchHomeData({DateTime? date}) async {
    ...
    try {
      ...
      final targetLeagues = leagues.where((l) => l.externalId == bundesligaId).toList();
      ...
      if (targetLeagues.isNotEmpty) {
        ...
        // Fallback segetStoredLeagues retornar lista ainda sem ID 78
        List<Fixture> matches = await apiClient.getFixturesByDate(bundesligaId, formattedDate);
        final dummyLeague = leagues.firstWhere((l) => l.externalId == bundesligaId, orElse: () => leagues.first);
