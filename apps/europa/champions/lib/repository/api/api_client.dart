...
class ApiClient {
  ...
  Future<List<Fixture>> getRecentFixtures(int leagueId, {int limit = 1, int? season}) async {
    final response = await http.get(Uri.parse('$baseUrl/fixtures?leagueId=$leagueId&limit=$limit&season=${season ?? AppConfig.defaultSeason}'));
    if (response.statusCode == 200) {
      final data = _decodeResponse(response);
      if (data is List) {
        return data.map((item) => Fixture.fromJson(item)).toList();
  ...
  Future<List<Standing>> getStandings(int leagueId, {int? season}) async {
    final response = await http.get(Uri.parse('$baseUrl/standings?leagueId=$leagueId&season=${season ?? AppConfig.defaultSeason}'));
    if (response.statusCode == 200) {
      final data = _decodeResponse(response);
      if (data is List) {
  ...
  Future<List<Scorer>> getScorers(int leagueId, {int? season}) async {
    final response = await http.get(Uri.parse('$baseUrl/competitions/$leagueId/scorers?season=${season ?? AppConfig.defaultSeason}'));
    if (response.statusCode == 200) {
      final data = _decodeResponse(response);
      if (data is List) {
    ...
    if (response.statusCode == 200) {
      ...
      if (data is List) {
        ...
        return list.where((f) {
          ...
          return extId == targetLeagueId || id == '$targetLeagueId' || AppConfig.supportedLeagueIds.contains(extId) || name.contains('champions');
        }).toList();
      }
      return [];
    } else {
