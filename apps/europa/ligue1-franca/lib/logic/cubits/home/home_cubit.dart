...
class HomeCubit extends Cubit<HomeState> {
  ...
  Future<void> fetchHomeData({DateTime? date}) async {
    ...
    try {
      ...
      final targetLeagues = leagues.where((l) => l.externalId == ligue1Id).toList();
      ...
      if (targetLeagues.isNotEmpty) {
        ...
        List<Fixture> matches = await apiClient.getFixturesByDate(ligue1Id, formattedDate);
        final dummyLeague = leagues.firstWhere((l) => l.externalId == ligue1Id, orElse: () => leagues.first);
