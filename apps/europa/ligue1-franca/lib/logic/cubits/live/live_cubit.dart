...
class LiveCubit extends Cubit<LiveState> {
  ...
  Future<void> fetchLiveFixtures({bool isAutoRefresh = false}) async {
    ...
    try {
      ...
      final targetId = AppConfig.leagueId;
      final fixtures = await apiClient.getLiveFixtures(leagueId: targetExtId);
      final ligue1Fixtures = fixtures.where((f) {
        final extId = f.league?.externalId;
        final id = f.league?.id;
        final name = f.league?.name.toLowerCase() ?? '';
        return extId == targetExtId || id == targetId || name.contains('ligue 1') || name.contains('ligue1');
      }).toList();
      emit(LiveLoaded(ligue1Fixtures));
    } catch (e) {
      if (!isAutoRefresh) {
