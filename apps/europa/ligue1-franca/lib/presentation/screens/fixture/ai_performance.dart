...
class _AiPerformanceDashboardPageState extends State<AiPerformanceDashboardPage> {
  ...
  Future<void> _loadLeagues() async {
    try {
      ...
      final targetExtId = AppConfig.externalLeagueId;
      final filtered = list.where((league) {
        final name = league.name.toLowerCase();
        final extId = league.externalId;
        return extId == targetExtId || name.contains('ligue 1') || name.contains('ligue1');
      }).toList();

      bool hasLigue1 = filtered.any((l) => l.externalId == targetExtId || l.name.toLowerCase().contains('ligue 1') || l.name.toLowerCase().contains('ligue1'));

      if (!hasLigue1) {
        filtered.insert(0, League(id: AppConfig.leagueId, externalId: targetExtId, name: 'Ligue 1', country: 'France'));
      }
      ...
      setState(() {
        _leagues = [
          League(id: AppConfig.leagueId, externalId: AppConfig.externalLeagueId, name: 'Ligue 1', country: 'France'),
        ];
      });
    }
  }
    ...
    try {
      ...
      if (data.recentAudits.isNotEmpty) {
        final filteredAudits = data.recentAudits.where((audit) {
          ...
          return lName.contains('ligue 1') || lName.contains('ligue1');
        }).toList();
