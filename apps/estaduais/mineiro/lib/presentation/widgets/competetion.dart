    ...
    return BlocBuilder<LeagueCubit, LeagueState>(
      builder: (context, state) {
        ...
        if (state is LeagueLoaded) {
          ...
          // 1. Fases Eliminatórias / Mata-Mata (Quartas, Semifinais, Final)
          ...
          // 2. Fases de Liga / Pontos Corridos / Grupos (1ª Fase, Quadrangular)
    ...
    return BlocBuilder<LeagueCubit, LeagueState>(
      builder: (context, state) {
        ...
        if (state is LeagueLoaded) {
          ...
          // 1. Fase de Grupos (Módulo I: Grupo A, B, C e Geral | Módulo II: Grupo A e Grupo B)
          if (activePhase != null && activePhase.isGroupStage && activePhase.groups.isNotEmpty) {
            ...
            final isModulo1 = state.selectedLeagueId == 629;
            final isGeneralGroup = currentGroup.id == 'classificacao_geral';
            ...
            if (isModulo1) {
              if (isGeneralGroup) {
                qualCount = 4;
                relCount = 2;
                qualDesc = 'Top 4 classificados para as Semifinais (3 líderes + melhor 2º colocado).';
                relDesc = '11º e 12º colocados são rebaixados para o Módulo II de 2027.';
              } else {
                qualCount = 1;
                relCount = 0;
                qualDesc = '1º colocado avança para as Semifinais (o melhor 2º geral entre os 3 grupos também classifica).';
                relDesc = null;
              }
            } else {
              // Módulo II
              qualCount = 4;
              relCount = 1;
              qualDesc = '1º ao 4º colocados avançam para as Quartas de Final (8 classificados).';
              relDesc = '6º colocado (último do grupo) é rebaixado para a Segunda Divisão de 2027.';
            }

            return ListView(
              padding: const EdgeInsets.only(left: 10, right: 10, top: 15, bottom: 120),
              children: [
                // Switcher de Grupos [Grupo A] [Grupo B] [Grupo C] [Geral]
                Container(
                ...
                StandingsTableWidget(
                  ...
                  qualificationDescription: qualDesc,
                  relegationDescription: relDesc,
                ),
              ],
            );
          }

          // 2. Fases de Mata-Mata / Eliminatórias (Quartas, Semis, Final)
          if (activePhase != null && activePhase.isKnockout) {
            return ListView(
              padding: const EdgeInsets.only(left: 10, right: 10, top: 15, bottom: 120),
              children: [
          ...
          // 3. Fase de Liga / Pontos Corridos
          if (activePhase != null && activePhase.isLeague) {
            final isM2 = state.selectedLeagueId == 619;
            return ListView(
              padding: const EdgeInsets.only(left: 10, right: 10, top: 15, bottom: 120),
              children: [
                StandingsTableWidget(
                  ...
                  qualificationCount: isM2 ? 8 : 4,
                  relegationCount: 2,
                  qualificationDescription: isM2
                      ? '1º ao 8º colocados avançam para as Quartas de Final.'
                      : '1º ao 4º colocados avançam para as Semifinais.',
                  relegationDescription: isM2
                      ? '11º e 12º colocados são rebaixados para a Segunda Divisão de 2027.'
                      : '11º e 12º colocados são rebaixados para o Módulo II de 2027.',
                ),
              ],
            );
          }
