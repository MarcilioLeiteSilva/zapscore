...
class SearchCubit extends Cubit<SearchState> {
  ...
  final Map<int, Team> _baianoTeamsMap = {};
  final Set<String> _baianoTeamNames = {};
  static const List<int> baianoIds = AppConfig.supportedLeagueIds; // [624, 851]

  SearchCubit(this.apiClient) : super(SearchInitial());

  Future<void> _loadBaianoTeams() async {
    if (_baianoTeamsMap.isNotEmpty) return;
    for (var lId in baianoIds) {
      try {
        final st = await apiClient.getStandings(lId);
        for (var s in st) {
          if (s.teamId > 0) {
            _baianoTeamsMap[s.teamId] = Team(
            ...
            if (s.teamName.isNotEmpty) {
              _baianoTeamNames.add(s.teamName.toLowerCase());
            }
          }
        }
      } catch (_) {}
      ...
      try {
        ...
        for (var f in fix) {
          if (f.homeTeam != null) {
            ...
            if (hId > 0) {
              _baianoTeamsMap[hId] = f.homeTeam!;
              if (f.homeTeam!.name.isNotEmpty) {
                _baianoTeamNames.add(f.homeTeam!.name.toLowerCase());
              }
            }
          }
          if (f.awayTeam != null) {
            ...
            if (aId > 0) {
              _baianoTeamsMap[aId] = f.awayTeam!;
              if (f.awayTeam!.name.isNotEmpty) {
                _baianoTeamNames.add(f.awayTeam!.name.toLowerCase());
              }
            }
          }
        }
  ...
  Future<void> search(String query) async {
    ...
    if (cleanQuery.isEmpty) {
      emit(SearchLoading());
      try {
        await _loadBaianoTeams();
        final leagues = await apiClient.getStoredLeagues();
        final baianoLeagues = leagues
            .where((l) =>
                baianoIds.contains(l.externalId) ||
                l.name.toLowerCase().contains('baiano'))
            .toList();
        emit(SearchLoaded(
          teams: _baianoTeamsMap.values.toList(),
          leagues: baianoLeagues.isNotEmpty ? baianoLeagues : leagues,
          fixtures: [],
        ));
      } catch (e) {
        emit(SearchInitial());
    ...
    try {
      ...
      try {
        ...
        // 1. Filtrar nos times conhecidos do Campeonato Baiano
        final Map<int, Team> filteredTeams = {};
        for (var team in _baianoTeamsMap.values) {
        ...
        // 2. Buscar times na API e filtrar estritamente pelos times do Campeonato Baiano
        try {
          ...
          for (var t in rawTeams) {
            ...
            final bool isBaiano = (_baianoTeamsMap.containsKey(extId)) ||
                _baianoTeamNames.any((cn) =>
                    cn == tName ||
