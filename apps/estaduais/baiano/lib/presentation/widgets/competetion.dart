    ...
    return BlocBuilder<LeagueCubit, LeagueState>(
      builder: (context, state) {
        ...
        if (state is LeagueLoaded) {
          ...
          // 1. Fases Eliminatórias / Mata-Mata (Semifinais e Final)
          ...
          // 2. Fases de Liga / Pontos Corridos (1ª Fase)
    ...
    return BlocBuilder<LeagueCubit, LeagueState>(
      builder: (context, state) {
        ...
        if (state is LeagueLoaded) {
          ...
          // 1. Fases de Mata-Mata / Eliminatórias (Semifinais e Final)
          if (activePhase != null && activePhase.isKnockout) {
            return ListView(
          ...
          // 2. Pontos Corridos / Liga Única (Série A e Série B)
          if (activePhase != null && activePhase.isLeague) {
            final isSerieA = state.selectedLeagueId == 602;
            return ListView(
              padding: const EdgeInsets.only(left: 10, right: 10, top: 15, bottom: 120),
              children: [
                StandingsTableWidget(
                  ...
                  relegationCount: isSerieA ? 2 : 0,
                  qualificationDescription: '1º ao 4º colocados avançam para as Semifinais.',
                  relegationDescription: isSerieA ? '9º e 10º colocados são rebaixados para a Série B de 2027.' : null,
                ),
              ],
            );
          }
