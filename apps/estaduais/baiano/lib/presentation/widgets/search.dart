  ...
  Widget build(BuildContext context) {
    return BlocBuilder<SearchCubit, SearchState>(
      builder: (context, state) {
        ...
        return Center(child: Text('type_to_search_matches'.tr(context)));
      },
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Ink(
        ...
        decoration: BoxDecoration(
          color: select ? const Color(0xFF454444) : Theme.of(context).cardColor.withOpacity(0.5),
        ...
        child: Center(
          child: Text(
            ...
            style: context.textTheme.bodySmall!.copyWith(
              color: select ? Colors.white : Colors.white70,
    ...
    return BlocBuilder<SearchCubit, SearchState>(
      builder: (context, state) {
        ...
        if (state is SearchLoaded) {
          ...
          if (items.isEmpty) {
            return Center(child: Text('no_popular_results'.tr(context)));
          }
          ...
          return Column(
            ...
            children: [
              Padding(
                padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 10),
                child: Text(
                  'main_results'.tr(context),
                  style: GoogleFonts.urbanist(
                    textStyle: context.textTheme.bodyMedium!.copyWith(color: const Color(0xFF1F2937), fontWeight: FontWeight.bold),
                  ),
                ),
              ),
              Expanded(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    if (item is League) {
                      return ListTile(
                        ...
                        title: Text(item.name, style: GoogleFonts.urbanist(color: const Color(0xFF1F2937), fontWeight: FontWeight.w600)),
                        subtitle: Text('competition'.tr(context), style: GoogleFonts.urbanist(color: const Color(0xFF1F2937).withOpacity(0.7))),
                        trailing: BlocBuilder<FavoriteCubit, FavoriteState>(
                          builder: (context, favState) {
                            ...
                            return Icon(isFav ? Icons.star : Icons.star_border, color: isFav ? const Color(0xFFAA7A13) : Colors.white, size: 18);
                          },
                        ),
                      );
        ...
        return Center(child: Text('type_to_search'.tr(context)));
      },
    );
  }
}
      ...
      builder: (context, state) {
        ...
        if (state is SearchLoaded) {
          if (state.leagues.isEmpty) {
            return Center(child: Text('no_competitions_found'.tr(context)));
          }
          return ListView.builder(
                  ...
                  children: [
                    Flexible(
                      child: Text(
                        ...
                        maxLines: 1,
                        style: GoogleFonts.urbanist(
                          textStyle: context.textTheme.bodyMedium!.copyWith(color: const Color(0xFF1F2937), fontWeight: FontWeight.w600),
                        ),
                      ),
                    ),
                    const Gap(5),
        ...
        return Center(child: Text('type_to_search_competitions'.tr(context)));
      },
    );
  }
}
      ...
      builder: (context, state) {
        ...
        if (state is SearchLoaded) {
          if (state.teams.isEmpty) {
            return Center(child: Text('no_teams_found'.tr(context)));
          }
          return ListView.builder(
        ...
        return Center(child: Text('type_to_search_teams'.tr(context)));
      },
    );
  }
}
      ...
      builder: (context, state) {
        ...
        if (state is VideoLoaded) {
          ...
          if (videoList.isEmpty) {
            return Center(child: Text('no_videos_found'.tr(context)));
          }
          return ListView.separated(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 20),
            itemBuilder: (_, i) {
