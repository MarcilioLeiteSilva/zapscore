...
class _HomeTopScorersCardState extends State<HomeTopScorersCard> {
  ...
  int _selectedLeagueId = AppConfig.paulistaSerieAId;
  final Map<int, List<Scorer>> _scorersCache = {};
  final Map<int, bool> _loadingMap = {};

  final List<Map<String, dynamic>> _leagues = [
    {'id': AppConfig.paulistaSerieAId, 'name': 'Série A1'},
    {'id': AppConfig.paulistaSerieA2Id, 'name': 'Série A2'},
  ];

  @override
  void initState() {
    super.initState();
    _fetchScorers(AppConfig.paulistaSerieAId);
  }

  Future<void> _fetchScorers(int leagueId) async {
    if (_scorersCache.containsKey(leagueId)) return;
    setState(() {
      _loadingMap[leagueId] = true;
    });
    try {
      final scorers = await _apiClient.getScorers(leagueId);
      if (mounted) {
        setState(() {
          _scorersCache[leagueId] = scorers;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Gap(10),
          // Abas de seleção de competição (Série A / Série A2)
          Container(
          ...
          if (isLoading)
          ...
          else if (currentScorers.isEmpty)
            Padding(
            ...
            Column(
              children: [
                for (int i = 0; i < (currentScorers.length > 3 ? 3 : currentScorers.length); i++) ...[
                  if (i > 0) const Divider(height: 12, color: Colors.white12),
                  CardTopScores(scorer: currentScorers[i], rank: i + 1),
                ],
              ],
            ),
        ],
          ...
          builder: (context, settingState) {
            ...
            return InkWell(
              ...
              child: Container(
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(30),
                  color: isSelected ? const Color(0xFF1B3765) : (isLive ? AppColor.accent.withOpacity(0.2) : Theme.of(context).cardColor.withOpacity(0.3)),
                  border: isSelected ? Border.all(color: AppColor.accent, width: 1.5) : null,
                ),
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: context.textTheme.bodySmall!.copyWith(
                        fontSize: 16,
                        color: (isSelected || isLive) ? AppColor.accent : Theme.of(context).colorScheme.onSurface,
...
class CardCalendarItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final textColor = select ? const Color(0xFF152A4E) : Theme.of(context).colorScheme.onSurface;
    ...
    return InkWell(
      ...
      child: Container(
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(12),
          color: select ? AppColor.accent : null,
        ),
        padding: const EdgeInsets.symmetric(
          horizontal: 5,
          vertical: 6,
