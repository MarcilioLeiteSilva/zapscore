  ...
  Future<void> _loadLeagues() async {
    try {
      ...
      final filtered = list.where((league) {
        ...
        return AppConfig.supportedLeagueIds.contains(extId) || name.contains('paulista');
      }).toList();

      bool hasPaulista = filtered.any((l) => AppConfig.supportedLeagueIds.contains(l.externalId) || l.name.toLowerCase().contains('paulista'));

      if (!hasPaulista) {
        filtered.insert(0, League(id: '475', externalId: 475, name: 'Paulista Série A1', country: 'Brazil'));
        filtered.add(League(id: '476', externalId: 476, name: 'Paulista Série A2', country: 'Brazil'));
      }
      ...
      setState(() {
        _leagues = [
          League(id: '475', externalId: 475, name: 'Paulista Série A1', country: 'Brazil'),
          League(id: '476', externalId: 476, name: 'Paulista Série A2', country: 'Brazil'),
        ];
      });
    }
  }
    ...
    try {
      ...
      if (data.recentAudits.isNotEmpty) {
        final filteredAudits = data.recentAudits.where((audit) {
          ...
          return lName.contains('paulista');
        }).toList();
