  ...
  Future<void> _loadLeagues() async {
    try {
      ...
      final filtered = list.where((league) {
        ...
        return AppConfig.supportedLeagueIds.contains(extId) || name.contains('paranaense');
      }).toList();

      bool hasParanaense = filtered.any((l) => AppConfig.supportedLeagueIds.contains(l.externalId) || l.name.toLowerCase().contains('paranaense'));

      if (!hasParanaense) {
        filtered.insert(0, League(id: '606', externalId: 606, name: 'Paranaense Série A', country: 'Brazil'));
        filtered.add(League(id: '614', externalId: 614, name: 'Paranaense Série B', country: 'Brazil'));
      }
      ...
      setState(() {
        _leagues = [
          League(id: '606', externalId: 606, name: 'Paranaense Série A', country: 'Brazil'),
          League(id: '614', externalId: 614, name: 'Paranaense Série B', country: 'Brazil'),
        ];
      });
    }
  }
    ...
    try {
      ...
      if (data.recentAudits.isNotEmpty) {
        final filteredAudits = data.recentAudits.where((audit) {
          ...
          return lName.contains('paranaense');
        }).toList();
