...
class SearchCubit extends Cubit<SearchState> {
  ...
  final Map<int, Team> _cearenseTeamsMap = {};
  final Set<String> _cearenseTeamNames = {};
  static const List<int> cearenseIds = AppConfig.supportedLeagueIds; // [607, 617]

  SearchCubit(this.apiClient) : super(SearchInitial());

  Future<void> _loadCearenseTeams() async {
    if (_cearenseTeamsMap.isNotEmpty) return;
    for (var lId in cearenseIds) {
      try {
        final st = await apiClient.getStandings(lId);
        for (var s in st) {
          if (s.teamId > 0) {
            _cearenseTeamsMap[s.teamId] = Team(
            ...
            if (s.teamName.isNotEmpty) {
              _cearenseTeamNames.add(s.teamName.toLowerCase());
            }
          }
        }
      } catch (_) {}
      ...
      try {
        ...
        for (var f in fix) {
          if (f.homeTeam != null) {
            ...
            if (hId > 0) {
              _cearenseTeamsMap[hId] = f.homeTeam!;
              if (f.homeTeam!.name.isNotEmpty) {
                _cearenseTeamNames.add(f.homeTeam!.name.toLowerCase());
              }
            }
          }
          if (f.awayTeam != null) {
            ...
            if (aId > 0) {
              _cearenseTeamsMap[aId] = f.awayTeam!;
              if (f.awayTeam!.name.isNotEmpty) {
                _cearenseTeamNames.add(f.awayTeam!.name.toLowerCase());
              }
            }
          }
        }
  ...
  Future<void> search(String query) async {
    ...
    if (cleanQuery.isEmpty) {
      emit(SearchLoading());
      try {
        await _loadCearenseTeams();
        final leagues = await apiClient.getStoredLeagues();
        final cearenseLeagues = leagues
            .where((l) =>
                cearenseIds.contains(l.externalId) ||
                l.name.toLowerCase().contains('cearense'))
            .toList();
        emit(SearchLoaded(
          teams: _cearenseTeamsMap.values.toList(),
          leagues: cearenseLeagues.isNotEmpty ? cearenseLeagues : leagues,
          fixtures: [],
        ));
      } catch (e) {
        emit(SearchInitial());
    ...
    try {
      ...
      try {
        ...
        // 1. Filtrar nos times conhecidos do Cearense Série A e Série B
        final Map<int, Team> filteredTeams = {};
        for (var team in _cearenseTeamsMap.values) {
        ...
        // 2. Buscar times na API e filtrar estritamente pelos times do Cearense
        try {
          ...
          for (var t in rawTeams) {
            ...
            final bool isCearense = (_cearenseTeamsMap.containsKey(extId)) ||
                _cearenseTeamNames.any((cn) =>
                    cn == tName ||
