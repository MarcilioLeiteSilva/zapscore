  ...
  Future<void> _loadLeagues() async {
    try {
      ...
      final filtered = list.where((league) {
        ...
        return AppConfig.supportedLeagueIds.contains(extId) || name.contains('carioca');
      }).toList();

      bool hasCarioca = filtered.any((l) => AppConfig.supportedLeagueIds.contains(l.externalId) || l.name.toLowerCase().contains('carioca'));

      if (!hasCarioca) {
        filtered.insert(0, League(id: '624', externalId: 624, name: 'Carioca Série A', country: 'Brazil'));
        filtered.add(League(id: '851', externalId: 851, name: 'Carioca Série A2', country: 'Brazil'));
      }
      ...
      setState(() {
        _leagues = [
          League(id: '624', externalId: 624, name: 'Carioca Série A', country: 'Brazil'),
          League(id: '851', externalId: 851, name: 'Carioca Série A2', country: 'Brazil'),
        ];
      });
    }
  }
