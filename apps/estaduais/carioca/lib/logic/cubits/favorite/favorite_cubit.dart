...
class FavoriteCubit extends Cubit<FavoriteState> {
  ...
  Future<void> fetchFavoriteDetails(
    ...
    try {
      ...
      final leagues = allLeagues.where((l) => leagueIds.contains(l.externalId.toString()) || leagueIds.contains(l.id)).toList();
      ...
      // Sincroniza os IDs numéricos oficiais com o PocketBase Europa
      final teamExtIds = teams.map((t) => t.externalId).where((id) => id > 0).toList();
      final fixtureExtIds = fixtures.map((f) => f.externalId).where((id) => id > 0).toList();
      PushNotificationService.syncDeviceWithPocketBase(
        favoriteTeamIds: teamExtIds,
        favoriteFixtureIds: fixtureExtIds,
      );
