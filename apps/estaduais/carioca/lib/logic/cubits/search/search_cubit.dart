...
class SearchCubit extends Cubit<SearchState> {
  ...
  final Map<int, Team> _cariocaTeamsMap = {};
  final Set<String> _cariocaTeamNames = {};
  static const List<int> cariocaIds = AppConfig.supportedLeagueIds; // [624, 851]

  SearchCubit(this.apiClient) : super(SearchInitial());

  Future<void> _loadCariocaTeams() async {
    if (_cariocaTeamsMap.isNotEmpty) return;
    for (var lId in cariocaIds) {
      try {
        final st = await apiClient.getStandings(lId);
        for (var s in st) {
          if (s.teamId > 0) {
            _cariocaTeamsMap[s.teamId] = Team(
            ...
            if (s.teamName.isNotEmpty) {
              _cariocaTeamNames.add(s.teamName.toLowerCase());
            }
          }
        }
      } catch (_) {}
      ...
      try {
        ...
        for (var f in fix) {
          if (f.homeTeam != null) {
            ...
            if (hId > 0) {
              _cariocaTeamsMap[hId] = f.homeTeam!;
              if (f.homeTeam!.name.isNotEmpty) {
                _cariocaTeamNames.add(f.homeTeam!.name.toLowerCase());
              }
            }
          }
          if (f.awayTeam != null) {
            ...
            if (aId > 0) {
              _cariocaTeamsMap[aId] = f.awayTeam!;
              if (f.awayTeam!.name.isNotEmpty) {
                _cariocaTeamNames.add(f.awayTeam!.name.toLowerCase());
              }
            }
          }
        }
  ...
  Future<void> search(String query) async {
    ...
    if (cleanQuery.isEmpty) {
      emit(SearchLoading());
      try {
        await _loadCariocaTeams();
        final leagues = await apiClient.getStoredLeagues();
        final cariocaLeagues = leagues
            .where((l) =>
                cariocaIds.contains(l.externalId) ||
                l.name.toLowerCase().contains('carioca'))
            .toList();
        emit(SearchLoaded(
          teams: _cariocaTeamsMap.values.toList(),
          leagues: cariocaLeagues.isNotEmpty ? cariocaLeagues : leagues,
          fixtures: [],
        ));
      } catch (e) {
        emit(SearchInitial());
    ...
    try {
      ...
      try {
        ...
        // 1. Filtrar nos times conhecidos do Carioca A1 e A2
        final Map<int, Team> filteredTeams = {};
        for (var team in _cariocaTeamsMap.values) {
        ...
        // 2. Buscar times na API e filtrar estritamente pelos times do Carioca
        try {
          ...
          for (var t in rawTeams) {
            ...
            final bool isCarioca = (_cariocaTeamsMap.containsKey(extId)) ||
                _cariocaTeamNames.any((cn) =>
                    cn == tName ||
      ...
      try {
        ...
        fixtures = rawFixtures.where((f) {
          ...
          return extId != null && cariocaIds.contains(extId);
        }).toList();
      } catch (e) {
