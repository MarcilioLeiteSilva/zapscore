...
class LiveCubit extends Cubit<LiveState> {
  ...
  Future<void> fetchLiveFixtures({bool isAutoRefresh = false}) async {
    ...
    try {
      ...
      const cariocaIds = AppConfig.supportedLeagueIds;
      final cariocaFixtures = fixtures.where((f) {
        ...
        return (extId != null && cariocaIds.contains(extId)) ||
            cariocaIds.any((cid) => id == '$cid') ||
            name.contains('carioca');
      }).toList();
      emit(LiveLoaded(cariocaFixtures));
    } catch (e) {
      if (!isAutoRefresh) {
